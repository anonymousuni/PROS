...
def comp_times(task, dev1, dev2, dev3):
    ...
    if (type(dev1) == list):
        dev1 = dev1[0]
    alloc_file = "D:\\00Research\\00Fog\\004-Zara\\Her SLA\\TimeIntervals-medium\\5\\Profs\\networkDefinition2.json"
    ...
    if (task == "encode_200"):
        ...
        Tr[0] = ((0.000001) * (video_size[index_of_segment]) / BW0) + ((0.000001) *LAT0)        
        T[0] = numpy.round(numpy.round(Tm[0], 4) +  numpy.round(Tr[0], 4), 4)
        #print (T[0])

        Tm[1] = encode_200[1]
        Tr[1] = ((0.000001) * (video_size[index_of_segment]) / BW1) + ((0.000001) *LAT1)        
        T[1] = numpy.round(numpy.round(Tm[1], 4) +  numpy.round(Tr[1], 4), 4)
        #print (T[1])

        Tm[2] = encode_200[2]
        Tr[2] = ((0.000001) * (video_size[index_of_segment]) / BW2) + ((0.000001) *LAT2)        
        T[2] = numpy.round(numpy.round(Tm[2], 4) +  numpy.round(Tr[2], 4), 4)
        #print (T[2])

        Tm[3] = encode_200[3]
        Tr[3] = ((0.000001) * (video_size[index_of_segment]) / BW3) + ((0.000001) *LAT3)        
        T[3] = numpy.round(numpy.round(Tm[3], 4) +  numpy.round(Tr[3], 4), 4)
        #print (T[3])

        Tm[4] = encode_200[4]
        Tr[4] = ((0.000001) * (video_size[index_of_segment]) / BW4) + ((0.000001) *LAT4)       
        T[4] = numpy.round(numpy.round(Tm[4], 4) +  numpy.round(Tr[4], 4), 4)
        #print (T[4])

        Tm[5] = encode_200[5]
        Tr[5] = ((0.000001) * (video_size[index_of_segment]) / BW5) + ((0.000001) *LAT5)        
        T[5] = numpy.round(numpy.round(Tm[5], 4) +  numpy.round(Tr[5], 4), 4)
        #print (T[5])

        Tm[6] = encode_200[6]
        Tr[6] = ((0.000001) * (video_size[index_of_segment]) / BW6) + ((0.000001) *LAT6)       
        T[6] = numpy.round(numpy.round(Tm[6], 4) +  numpy.round(Tr[6], 4), 4)
        #print (T[6])

        Tm[7] = encode_200[7]
        Tr[7] = ((0.000001) * (video_size[index_of_segment]) / BW7) + ((0.000001) *LAT7)       
        T[7] = numpy.round(numpy.round(Tm[7], 4) +  numpy.round(Tr[7], 4), 4)

        Tm[8] = encode_200[8]
        Tr[8] = ((0.000001) * (video_size[index_of_segment]) / BW8) + ((0.000001) *LAT8)         
        T[8] = numpy.round(numpy.round(Tm[8], 4) +  numpy.round(Tr[8], 4), 4)


        Tm[9] = encode_200[9]
        Tr[9] = ((0.000001) * (video_size[index_of_segment]) / BW9) + ((0.000001) *LAT9) 
        T[9] = numpy.round(numpy.round(Tm[9], 4) + numpy.round(Tr[9], 4), 4)


        Tm[10] = encode_200[10]
        Tr[10] = ((0.000001) * (video_size[index_of_segment]) / BW10) + ((0.000001) *LAT10) 
        T[10] = numpy.round(numpy.round(Tm[10], 4) +  numpy.round(Tr[10], 4), 4)
        # print (T[10])

        Tm[11] = encode_200[11]
        Tr[11] = ((0.000001) * (video_size[index_of_segment]) / BW11) + ((0.000001) *LAT11) 
        T[11] = numpy.round(numpy.round(Tm[11], 4) +  numpy.round(Tr[11], 4), 4)
        # print (T[11])
        ...
        Tr[0] = max(((0.000001) * (video_size[index_of_segment]) / BW0) + ((0.000001) *LAT0),
                ((0.000001) * (video_size[index_of_segment]) / BW20) + ((0.000001) *LAT20))  
        T[0] = numpy.round(numpy.round(Tm[0], 4) +  numpy.round(Tr[0], 4), 4)
        # print (T[0][0])

        Tm[1] = inference[1]
        Tr[1] = max(((0.000001) * (video_size[index_of_segment]) / BW1) + ((0.000001) *LAT1),
                ((0.000001) * (video_size[index_of_segment]) / BW21) + ((0.000001) *LAT21))         
        T[1] = numpy.round(numpy.round(Tm[1], 4) +  numpy.round(Tr[1], 4), 4)
        # print (T[1])

        Tm[2] = inference[2]
        Tr[2] = max(((0.000001) * (video_size[index_of_segment]) / BW2) + ((0.000001) *LAT2),
                ((0.000001) * (video_size[index_of_segment]) / BW22) + ((0.000001) *LAT22))         
        T[2] = numpy.round(numpy.round(Tm[2], 4) +  numpy.round(Tr[2], 4), 4)
        # print (T[2])

        Tm[3] = inference[3]
        Tr[3] = max(((0.000001) * (video_size[index_of_segment]) / BW3) + ((0.000001) *LAT3),
                ((0.000001) * (video_size[index_of_segment]) / BW23) + ((0.000001) *LAT23))         
        T[3] = numpy.round(numpy.round(Tm[3], 4) +  numpy.round(Tr[3], 4), 4)
        # print (T[3])

        Tm[4] = inference[4]
        Tr[4] = max(((0.000001) * (video_size[index_of_segment]) / BW4) + ((0.000001) *LAT4),
                ((0.000001) * (video_size[index_of_segment]) / BW24) + ((0.000001) *LAT24))        
        T[4] = numpy.round(numpy.round(Tm[4], 4) +  numpy.round(Tr[4], 4), 4)
        # print (T[4])

        Tm[5] = inference[5]
        Tr[5] = max(((0.000001) * (video_size[index_of_segment]) / BW5) + ((0.000001) *LAT5),
                ((0.000001) * (video_size[index_of_segment]) / BW25) + ((0.000001) *LAT25))
        T[5] = numpy.round(numpy.round(Tm[5], 4) +  numpy.round(Tr[5], 4), 4)
        # print (T[5])

        Tm[6] = inference[6]
        Tr[6] = max(((0.000001) * (video_size[index_of_segment]) / BW6) + ((0.000001) *LAT6),
                ((0.000001) * (video_size[index_of_segment]) / BW26) + ((0.000001) *LAT26)) 
        T[6] = numpy.round(numpy.round(Tm[6], 4) +  numpy.round(Tr[6], 4), 4)
        # print (T[6])

        Tm[7] = inference[7]
        Tr[7] = max(((0.000001) * (video_size[index_of_segment]) / BW7) + ((0.000001) *LAT7),
                ((0.000001) * (video_size[index_of_segment]) / BW27) + ((0.000001) *LAT27)) 
        T[7] = numpy.round(numpy.round(Tm[7], 4) +  numpy.round(Tr[7], 4), 4)
        # print (T[7])


        Tm[8] = inference[8]
        Tr[8] = max(((0.000001) * (video_size[index_of_segment]) / BW8) + ((0.000001) *LAT8),
                ((0.000001) * (video_size[index_of_segment]) / BW28) + ((0.000001) *LAT28))
        T[8] = numpy.round(numpy.round(Tm[8], 4) +  numpy.round(Tr[8], 4), 4)
        # print (T[8])

        Tm[9] = inference[9]
        Tr[9] = max(((0.000001) * (video_size[index_of_segment]) / BW9) + ((0.000001) *LAT9),
                ((0.000001) * (video_size[index_of_segment]) / BW29) + ((0.000001) *LAT29))
        T[9] = numpy.round(numpy.round(Tm[9], 4) + numpy.round(Tr[9], 4), 4)
        # print (T[9])

        Tm[10] = inference[10]
        Tr[10] = max(((0.000001) * (video_size[index_of_segment]) / BW10) + ((0.000001) *LAT10),
                ((0.000001) * (video_size[index_of_segment]) / BW210) + ((0.000001) *LAT210))        
        T[10] = numpy.round(numpy.round(Tm[10], 4) +  numpy.round(Tr[10], 4), 4)
        # print (T[10])

        
        Tm[11] = inference[11]
        Tr[11] = max(((0.000001) * (video_size[index_of_segment]) / BW11) + ((0.000001) *LAT11),
                ((0.000001) * (video_size[index_of_segment]) / BW211) + ((0.000001) *LAT211))        
        T[11] = numpy.round(numpy.round(Tm[11], 4) +  numpy.round(Tr[11], 4), 4)
        # print (T[11])
        ...
        Tr[0] = max(((0.000001) * (video_size[index_of_segment]) / BW0) + ((0.000001) *LAT0),
                ((0.000001) * (video_size[index_of_segment]) / BW20) + ((0.000001) *LAT20),
                ((0.000001) * (video_size[index_of_segment]) / BW30) + ((0.000001) *LAT30))
        T[0] = numpy.round(numpy.round(Tm[0], 4) +  numpy.round(Tr[0], 4), 4)
        #print (T[0])
        ...
        Tr[1] = max(((0.000001) * (video_size[index_of_segment]) / BW1) + ((0.000001) *LAT1),
                ((0.000001) * (video_size[index_of_segment]) / BW21) + ((0.000001) *LAT21),
                ((0.000001) * (video_size[index_of_segment]) / BW31) + ((0.000001) *LAT31))
        T[1] = numpy.round(numpy.round(Tm[1], 4) +  numpy.round(Tr[1], 4), 4)
        #print (T[1])
        ...
        Tr[2] = max(((0.000001) * (video_size[index_of_segment]) / BW2) + ((0.000001) *LAT2),
                ((0.000001) * (video_size[index_of_segment]) / BW22) + ((0.000001) *LAT22),
                ((0.000001) * (video_size[index_of_segment]) / BW32) + ((0.000001) *LAT32))
        T[2] = numpy.round(numpy.round(Tm[2], 4) +  numpy.round(Tr[2], 4), 4)
        #print (T[2])
        ...
        Tr[3] = max(((0.000001) * (video_size[index_of_segment]) / BW3) + ((0.000001) *LAT3),
                ((0.000001) * (video_size[index_of_segment]) / BW23) + ((0.000001) *LAT23),
                ((0.000001) * (video_size[index_of_segment]) / BW33) + ((0.000001) *LAT33))
        T[3] = numpy.round(numpy.round(Tm[3], 4) +  numpy.round(Tr[3], 4), 4)
        #print (T[3])
        ...
        Tr[4] = max(((0.000001) * (video_size[index_of_segment]) / BW4) + ((0.000001) *LAT4),
                ((0.000001) * (video_size[index_of_segment]) / BW24) + ((0.000001) *LAT24),
                ((0.000001) * (video_size[index_of_segment]) / BW34) + ((0.000001) *LAT34))
        T[4] = numpy.round(numpy.round(Tm[4], 4) +  numpy.round(Tr[4], 4), 4)
        #print (T[4])
        ...
        Tr[5] = max(((0.000001) * (video_size[index_of_segment]) / BW5) + ((0.000001) *LAT5),
                ((0.000001) * (video_size[index_of_segment]) / BW25) + ((0.000001) *LAT25),
                ((0.000001) * (video_size[index_of_segment]) / BW35) + ((0.000001) *LAT35))
        T[5] = numpy.round(numpy.round(Tm[5], 4) +  numpy.round(Tr[5], 4), 4)
        #print (T[5])
        ...
        Tr[6] = max(((0.000001) * (video_size[index_of_segment]) / BW6) + ((0.000001) *LAT6),
                ((0.000001) * (video_size[index_of_segment]) / BW26) + ((0.000001) *LAT26),
                ((0.000001) * (video_size[index_of_segment]) / BW36) + ((0.000001) *LAT36))
        T[6] = numpy.round(numpy.round(Tm[6], 4) +  numpy.round(Tr[6], 4), 4)
        #print (T[6])
        ...
        Tr[7] = max(((0.000001) * (video_size[index_of_segment]) / BW7) + ((0.000001) *LAT7),
                ((0.000001) * (video_size[index_of_segment]) / BW27) + ((0.000001) *LAT27),
                ((0.000001) * (video_size[index_of_segment]) / BW37) + ((0.000001) *LAT37))
        T[7] = numpy.round(numpy.round(Tm[7], 4) +  numpy.round(Tr[7], 4), 4)

        Tm[8] = encode_200[8]
        Tr[8] = max(((0.000001) * (video_size[index_of_segment]) / BW8) + ((0.000001) *LAT8),
                ((0.000001) * (video_size[index_of_segment]) / BW28) + ((0.000001) *LAT28),
                ((0.000001) * (video_size[index_of_segment]) / BW38) + ((0.000001) *LAT38))
        T[8] = numpy.round(numpy.round(Tm[8], 4) +  numpy.round(Tr[8], 4), 4)
